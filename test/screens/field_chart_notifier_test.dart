...
void main() {
  ...
  group('truncated coverage', () {
    test('refetches and still reports truncated when the same truncated range '
        'is requested again', () async {
      final notifier = await settleNotifier(const Field(id: 1, label: 'Temp'));
    ...
    test('fetches rather than reading from cache for a subrange older than '
        'coveredFrom', () async {
      final notifier = await settleNotifier(const Field(id: 1, label: 'Temp'));
    ...
    test('serves a subrange within [coveredFrom, range.end] from cache after a '
        'truncated fetch', () async {
      final notifier = await settleNotifier(const Field(id: 1, label: 'Temp'));
    ...
    test('a non-truncated empty window still records coverage and is not '
        'refetched (regression guard)', () async {
      final notifier = await settleNotifier(const Field(id: 1, label: 'Temp'));
      final range = DateTimeRange(start: twoDaysAgo, end: now);
