...
- A successful (HTTP 200) response carrying something other than usable feed data left the channel detail, field chart and pinned-field screens stuck on a spinner with no error and no way to retry. This covers an HTML error or captive-portal page, truncated JSON, and a ThingSpeak error object delivered under a 200 status. Such a response is now reported as a readable error, an auth error object under 200 is recognised as a credentials failure, and a single junk feed entry no longer discards the rest of an otherwise good response.
...
- The channel detail screen could show or persist older data after an edit or an overlapping refresh: when two fetches overlapped, or the channel was edited while a fetch was in flight, the older response could replace the newer values on screen and in the cached snapshot, and report obsolete metadata or a stale credentials failure back to the channel list. Responses from superseded requests are now dropped before any side effect.
- Pinned fields on the channel list could reappear after being unpinned or after their channel was deleted, when a pinned refresh was already in flight and landed afterward. The same stale response could also re-write a deleted channel's cached values back to storage. Superseded pinned refreshes are now dropped before touching the entries shown on screen or the snapshot cache.
